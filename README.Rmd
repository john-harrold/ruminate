...
You can install the released version of ``ruminate`` from [CRAN](https://cran.r-project.org/package=ruminate) with:
...
Make sure you install `{formods}` first and make sure to set dependencies to `TRUE` for both. 
...
devtools::install_github("humanpred/pknca",        dependencies=TRUE)
...
Note that because `{ruminate}` depends on `{formods}` and `{onbrand}` you will need to first install the development versions of `{onbrand}` and `{formods}`. You may also need to install the development version of `{PKNC}` as well. 
...
As new modules are developed they can be found in the development app. You can access them by setting `devmode` to `TRUE`:
...
If you want to deploy and customize ruminate, please see the [Deployment vignette](https://ruminate.ubiquity.tools/articles/deployment.html).
