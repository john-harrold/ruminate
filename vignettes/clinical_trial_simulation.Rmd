...
in the preamble and use those functions in the methods above. Alternatively you
can create your own code and use the `"manual"` rule type to modify the event
table using the objects in the rule evaluation environment above. In the code
below we are simply setting a state for the current subject id to a value. Note
that when you define the events tabel there are a couple things you need to do.
You need to use the `rbind()` function to append events. You must also specify
values for all of these columns:
...
```{r echo=TRUE, results=TRUE, eval=FALSE}
code="
SI_interval_ev = rbind(SI_interval_ev,
et(cmt  = 'Ac',
   id   = id,
   amt  = 10,
   time = time
